...
def tfm_score(df: pd.DataFrame, case_col: str, group_cols: list[str],
              return_rows: bool = False) -> dict:
    name, forecast = get_forecaster()
    keys = group_cols + ["YR"]
    ys, scores, ckeys, wns = [], [], [], []
    for kv, sub in df.groupby(keys, sort=False):
        sub = sub.sort_values("WN")
        cases = sub[case_col].to_numpy(dtype=float)
        lab = sub["labelable"].to_numpy()
        y = sub["y"].to_numpy()
        wn = sub["WN"].to_numpy() if "WN" in sub else np.arange(len(sub))
        # cluster id = unit-year, matching evaluate.loyo's key construction
        if group_cols:
            kvt = kv if isinstance(kv, tuple) else (kv,)
            unit = "-".join(str(x) for x in kvt[:-1])
            yr = int(kvt[-1])
            ck = f"{unit}-{yr}" if unit else str(yr)
        else:
            ck = str(int(kv if not isinstance(kv, tuple) else kv[-1]))
        for t in range(len(sub)):
            ...
            ckeys.append(ck); wns.append(int(wn[t]))
    ys, scores = np.array(ys), np.array(scores)
    if len(np.unique(ys)) < 2:
        res = dict(name=name, auroc=float("nan"), auprc=float("nan"), n=int(len(ys)))
    else:
        res = dict(name=name, auroc=float(roc_auc_score(ys, scores)),
                   auprc=float(average_precision_score(ys, scores)), n=int(len(ys)))
    if return_rows:
        return res, dict(cluster=np.array(ckeys, dtype=object), wn=np.array(wns),
                         y=ys, score=scores)
    return res
