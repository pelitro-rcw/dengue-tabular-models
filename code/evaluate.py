...
def _seed() -> int:
    """Active seed. Overridable via PIPELINE_SEED so the stability runner can
    vary the pipeline's own randomness (train subsample + backend) and quantify
    Monte-Carlo variance separately from the data bootstrap."""
    return int(os.environ.get("PIPELINE_SEED", str(RNG)))
...
def get_backend():
        ...
        try:
            ...
            def fp(Xtr, ytr, Xte):
                clf = TabPFNClassifier(n_estimators=n_est, device="cpu",
                                       balance_probabilities=True, random_state=_seed())
                clf.fit(Xtr, ytr)
    ...
    def fp(Xtr, ytr, Xte):
        m = HistGradientBoostingClassifier(random_state=_seed(), class_weight="balanced")
        m.fit(Xtr, ytr)
...
def _prep(Xtr, Xte):
    ...
    qt = QuantileTransformer(output_distribution="normal",
                             n_quantiles=min(1000, len(a)), random_state=_seed()).fit(a)
...
def loyo(df: pd.DataFrame, cols: list[str], backend_fp, return_oof: bool = False,
    ...
    for y in years[2:]:                       # expanding: need >=2 train years
        ...
        # cluster id = unit-year (the resampling unit for the cluster bootstrap)
        if group_cols:
            ck = (te[group_cols].astype(str).agg("-".join, axis=1) + "-" + str(int(y))).to_numpy()
        else:
            ck = np.full(len(te), str(int(y)), dtype=object)
        C.append(ck)
        W.append(te["WN"].to_numpy() if "WN" in te else np.arange(len(te)))
    Y, P = np.concatenate(Y), np.concatenate(P)
    C = np.concatenate(C) if C else np.array([], dtype=object)
    W = np.concatenate(W) if W else np.array([])
...
def loyo_per_region(df: pd.DataFrame, cols: list[str], group_col: str, backend_fp,
                    climate_map: dict | None = None, return_oof: bool = False):
    """Per-unit leave-one-year-out metrics, with optional climate-type tag.
    If return_oof, also returns {unit: (y_true, y_prob, year_cluster)} so per-unit
    CIs can be bootstrapped from the SAME predictions the point AUROC came from."""
    rows = []; oof = {}
    for unit, sub in df.groupby(group_col):
        try:
            if return_oof:
                r, Yi, Pi, Ci, Wi = loyo(sub, cols, backend_fp, return_oof=True,
                                         group_cols=[group_col])
                # Ci is "<unit>-<year>"; the per-unit resampling cluster is the year
                yr = np.array([c.rsplit("-", 1)[-1] for c in Ci.astype(str)])
                oof[str(unit)] = (Yi, Pi, yr)
            else:
    ...
    return (per, oof) if return_oof else per
